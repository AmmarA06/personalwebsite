...
function ProjectCard({ project }) {
    ...
    const io = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          el.playbackRate = project.playbackSpeed || 1.0;
          el.play().catch(() => {});
        } else {
          el.pause();
        }
  ...
  }, [project.mediaType, project.playbackSpeed]);

  const images = Array.isArray(project.media) ? project.media : project.media ? [project.media] : [];
  const hasMultipleImages = images.length > 1;
  const posterSrc =
    project.mediaType === "video" && typeof project.media === "string"
      ? project.media.replace("/videos/", "/videos/posters/").replace(/\.mp4$/, ".jpg")
      : undefined;
